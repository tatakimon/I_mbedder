...
int main(void)
{
  ...
  /* USART3 on PD8/PD9 — Logic Analyzer probe point */
  ...
  HAL_UART_Transmit(&huart3, ready_msg, ready_len, HAL_MAX_DELAY);

  /* HTS221 temperature sensor on I2C1, instance 0 */
  ...
  while (1) {
    ...
    uint8_t msg[32];
    int len = snprintf((char *)msg, sizeof(msg), "Temp: %.1f C\r\n", temp_value);
